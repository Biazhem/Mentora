...
function FounderForm({ formData, updateField, onFounderPhotoChange }) {
  return (
    <div className="space-y-2">
      <div>
...
export default function Page() {
  ...
  const handleLogoChange = (file) => {
    if (!file) return;
    setLogoFile(file);
    updateField("orgLogo", URL.createObjectURL(file));
  };

  const handleFounderPhotoChange = (file) => {
    if (!file) return;
    setFounderPhotoFile(file);
    updateField("founderPhoto", URL.createObjectURL(file));
  };
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const [orgLogoUrl, founderPhotoUrl] = await Promise.all([
        logoFile ? fileToBase64(logoFile) : "",
        founderPhotoFile ? fileToBase64(founderPhotoFile) : "",
      ]);
